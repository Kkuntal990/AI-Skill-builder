...
> **Experimental delivery arms (2026-07-18, additive).** Beyond the two default arms above — **with-skill (legacy injection)** and **without-skill** — an experimental **capability-injection** treatment is now available for the capability-linker study: `capability_task` and `capability_node` (via `MLEVAL_SKILL_DELIVERY_MODE`, sidecar 1.1.0), which link source-grounded *capability units* instead of whole `SKILL.md` bodies. `legacy` stays the default and the control. The method's live A/B (E3) has four possible arms — `no_skill` / `legacy_node` / `capability_task` / `capability_node` — but the **first live run is simplified to `capability_node` only**, reusing the existing mvp-032 legacy / `no_skill` baselines after a one-trajectory reproduction check on the rebuilt image (the full four-arm matrix is not run from the start). Design + gates E0–E3: [capability-linker plan](../skill-builder/capability-linker-mvp-plan.md) (HLD: [capability-linker-mvp-hld.md](../skill-builder/capability-linker-mvp-hld.md)); the runtime delivery mechanism sits alongside the legacy per-node selector in [skill-retrieval-design.md](./skill-retrieval-design.md).
...
**Contract coverage (2026-07):** generated skills now declare `metadata.openclaw.mcps.on_unavailable` — an explicit instruction to answer from `SKILL.md` + `references/` and state uncertainty rather than fabricate when no MCP client is reachable. This makes the MLEvolve path (no MCP client) a **declared, graceful-degrade** case rather than an unstated gap: the skill's MCP promises are conditional by contract. *Deferred future work* (not blocking skill-creation): giving MLEvolve a real MCP client via an HTTP-shim sidecar around `mcp.context7.com` so the fallback path can actually fire; until then `on_unavailable` is the honest behavior.
