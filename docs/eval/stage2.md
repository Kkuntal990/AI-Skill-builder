...
**Tool surface — no MCP, no web, by design.** The harness inherits AIDE's tool surface: LLM emits Python, `Interpreter` executes it, captured stdout/stderr feeds the next call. AIDE's only function-calling use is structured-output extraction of its `submit_review` schema — there is no general tool dispatch, no MCP client, no `web_search`. This matches the dominant convention across Kaggle-style MLE benchmarks: MLE-Bench's `rule_violation_detector` (`UNAUTHORIZED_ACCESS_QUESTION`) explicitly flags as a violation any agent attempt to access external resources outside what was officially provided; MLAgentBench's ResearchAgent has a closed hand-coded action set with no web/MCP; DSBench ships offline bundles. The motivations: reproducibility (network responses drift), test-set contamination avoidance (Kaggle top-solution write-ups are indexable), and cross-LLM fairness (many LLMs ship without browsing). Consequence for skills authored against an OpenClaw runtime (e.g., `vllm-inference`'s `context7__*` MCP fallback hooks): those paths are unreachable in this harness. The bundled `references/*.md` content carries the load the MCP fallbacks would otherwise fetch — which is precisely why progressive-disclosure skills front-load deep material into references rather than relying solely on runtime lookup. Sources: [openai/mle-bench](https://github.com/openai/mle-bench) (`extras/rule_violation_detector/prompts.py`), [snap-stanford/MLAgentBench](https://github.com/snap-stanford/MLAgentBench), [DSBench](https://arxiv.org/abs/2409.07703).
