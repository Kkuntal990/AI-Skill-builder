...
**1. Functional A/B (`functional` / `all`)** — both cells run through `openclaw agent --json`; the with-skill cell appends a `(… a skill is installed at <dir>. Read SKILL.md …)` marker, the without-skill cell sends the bare prompt. Grading is deterministic — `_score_assertions` does literal `must_contain` / `must_contain_any` / `must_not_contain` / `expected_citations` matching, no LLM judge on this path. So it measures *"does the skill's content help once the agent reads it,"* not whether the agent finds it. **As of 2026-07 this also runs inside the `full` ship-gate as an ADVISORY signal** (baseline executor = `main`; the builder auto-generates the assertions and `analyze()` flags non-discriminating/flaky ones) — it reports pass-rate + lift but never hard-blocks the ship, since the assertions are self-authored. Not the GPU task-solving A/B (that's Stage 2).
