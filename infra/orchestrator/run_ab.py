...
class Trajectory:
    ...
    # Per-node skill-injection caps (sidecar >= 1.0.0). Default 3/3 matches the
    # SkillsBench 2–3-skill optimum; 0 = inject none (catalog-only ablation).
    skill_max_per_node: int = 3
    skill_max_refs_per_node: int = 3
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--skill-max-per-node", type=int, default=3, help="Cap on skill BODIES injected per codegen node (sidecar >= 1.0.0). 0 = catalog-only. Applies to both cells symmetrically.")
    p.add_argument("--skill-max-refs-per-node", type=int, default=3, help="Cap on skill REFERENCES injected per codegen node (total across skills).")
    p.add_argument(
        "--profile",
        choices=["gpu"],
        default="gpu",
