...
Claude Opus 4.6 via OpenRouter. API key from `OPENROUTER_API_KEY` env var or
OpenClaw auth store (`openrouter:default` profile). Script exits clearly if
missing -- synthesis has no useful fallback.
